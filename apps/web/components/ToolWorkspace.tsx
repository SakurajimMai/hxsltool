...
async function localProcess(files: File[], tool: Tool, options: Record<string, string>): Promise<File> {
  ...
  if (tool.slug === "svg-qr-code") return new File([await QRCode.toString(options.text || "https://example.com", { type: "svg", errorCorrectionLevel: (options.errorCorrection || "M") as "L" | "M" | "Q" | "H" })], "qr-code.svg", { type: "image/svg+xml" });
